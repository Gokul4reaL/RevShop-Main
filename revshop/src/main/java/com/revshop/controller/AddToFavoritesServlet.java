...
public class AddToFavoritesServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            Favorites favorite = new Favorites(userId, productId);

            // Add the favorite item via FavoritesService
            favoritesService.addFavoriteItem(favorite);

            // Send success response
            response.setStatus(HttpServletResponse.SC_OK);
            response.getWriter().write("{\"message\":\"Product added to favorites successfully!\"}");
            ...
            response.getWriter().write(String.format("{\"error\":\"Failed to add product to favorites: %s\"}", e.getMessage()));
        }
    }
}
